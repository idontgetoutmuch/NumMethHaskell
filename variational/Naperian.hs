...
instance Naperian Triple where
  ...
  lookup (Triple x _ _) One = x
  lookup (Triple _ y _) Two = y
  lookup (Triple _ _ z) Three = z
  positions = Triple One Two Three
...
instance               Dimension Triple     where size = const 3
...
zzH = Prism (Prism (Prism (Scalar zz)))
